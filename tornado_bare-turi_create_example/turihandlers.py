...
class UpdateModelForDatasetId(BaseHandler):
    def get(self):
        ...
        if len(data)>0:
            ...
            model.save('/Users/evelynxu/Documents/SMU/MSL/turimodels/model%d'%(dsid))
...
class PredictOneFromDatasetId(BaseHandler):
    def post(self):
        ...
        # load the model from documents
        self.clf = tc.load_model('/Users/evelynxu/Documents/SMU/MSL/turimodels/model%d'%(dsid))
  
        probability = self.clf.predict(fvals,output_type = 'probability')
        predLabel = self.clf.predict(fvals)

        if (probability[0]>0.5 and predLabel[0] == 'Switch') or (probability[0]<0.3 and predLabel[0] == 'Ah'):
            predLabel = self.clf.predict(fvals)
        else:
            predLabel = ['none']
        ...
        # Test
        probability = self.clf.predict(fvals,output_type = 'probability')
        print("probability",probability,"label",predLabel)
